...
{
    /// <summary>
    /// Reference Assigner for CashBattleOnboarding scene.
    /// Automatically finds and assigns UI references to CashBattleOnboardingManager.
    ///
    /// Menu: DigitPark/Auto Assigners/References/Onboarding/CashBattleOnboarding References
    /// </summary>
    public class CashBattleOnboardingReferenceAssigner : EditorWindow
    {
        ...
        private static readonly string[] REQUIRED_REFS = {
            "slidesContainer",
            "nextButton", "backButton", "skipButton", "nextButtonText",
            "dotsContainer"
            // Note: progressBar excluded - optional UI element not in scene
        };
        ...
        [MenuItem("DigitPark/Scenes/Assign References/Onboarding/CashBattle", false, 171)]
        ...
        private void OnGUI()
        {
            GUILayout.Label("CashBattleOnboarding Scene Reference Assigner", EditorStyles.boldLabel);
            GUILayout.Space(10);

            string currentScene = UnityEngine.SceneManagement.SceneManager.GetActiveScene().name;
            if (currentScene != "CashBattleOnboarding")
            {
                EditorGUILayout.HelpBox(
                    $"Current scene: {currentScene}\n" +
                    "Please open the CashBattleOnboarding scene first!",
                    MessageType.Warning);
            }

            EditorGUILayout.HelpBox(
                "Assigns UI references to CashBattleOnboardingManager:\n" +
                "• Slides container\n" +
            ...
            MonoBehaviour targetManager = FindCashBattleOnboardingManager();
        ...
        {
            Log("=== ASSIGNING CASHBATTLEONBOARDING REFERENCES ===");

            var manager = FindCashBattleOnboardingManager();
            if (manager == null)
            {
                Log("ERROR: CashBattleOnboardingManager not found in scene!");
            ...
            // Slides
            AssignReference(so, "slidesContainer", FindByNameContains<Transform>("slidescontainer", "slides", "container"));

            // Navigation buttons
            AssignReference(so, "nextButton", FindButtonByName("next", "siguiente", "start"));
            AssignReference(so, "backButton", FindButtonByName("back", "anterior", "prev"));
            AssignReference(so, "skipButton", FindButtonByName("skip", "saltar", "omitir"));
            AssignReference(so, "nextButtonText", FindTextByName("nextbutton", "starttext"));

            // Dots
            AssignReference(so, "dotsContainer", FindByNameContains<Transform>("dotscontainer", "dots", "indicators"));
        ...
        private static MonoBehaviour FindCashBattleOnboardingManager()
        {
            foreach (var mb in Object.FindObjectsOfType<MonoBehaviour>(true))
                if (mb.GetType().Name == "CashBattleOnboardingManager") return mb;
            return null;
        }
        ...
        private static void Log(string msg) { log += msg + "\n"; Debug.Log($"[CashBattleOnboardingReferenceAssigner] {msg}"); }
