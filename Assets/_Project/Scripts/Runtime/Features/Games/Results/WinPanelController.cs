...
namespace DigitPark.UI
{
    ...
    public class WinPanelController : MonoBehaviour
    {
        ...
        public void ShowRealMoneyResult(MinigameResult playerResult, MinigameResult opponentResult,
            decimal entryFee, bool playerWon, string opponentName = null)
        {
            isWinner = playerWon;
            moneyWon = playerWon ? entryFee * 1.8m : 0; // 90% del pool (10% comisión)

            StartCoroutine(ShowRealMoneySequence(playerResult, opponentResult, entryFee, opponentName ?? AutoLocalizer.Get("opponent")));
        }
