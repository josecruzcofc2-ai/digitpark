...
namespace DigitPark.Managers
{
    ...
    public class MatchHistorySceneManager : MonoBehaviour
    {
            ...
            if (currentOffset == 0 && (entries == null || entries.Count == 0))
            {
                if (emptyText != null)
                {
                    emptyText.text = currentFilter != null
                        ? AutoLocalizer.Get("match_history_no_games", GetGameDisplayName(currentFilter))
                        : AutoLocalizer.Get("match_history_no_games_yet");
        ...
        private string GetGameDisplayName(string gameType)
        {
            ...
            {
                "DigitRush" => AutoLocalizer.Get("game_display_digit_rush"),
                "MemoryPairs" => AutoLocalizer.Get("game_display_memory_pairs"),
