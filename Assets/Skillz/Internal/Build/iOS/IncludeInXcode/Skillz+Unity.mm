...
static const BOOL REPLAY_LOG_ENABLED = NO; 

void ReplayLog(NSString *format, ...) {
    if (REPLAY_LOG_ENABLED) {
        va_list args;
...
- (id)init
{
    ReplayLog(@"[SKZRenderDelegate] init");
...
- (void)dealloc
{
    ReplayLog(@"[SKZRenderDelegate] dealloc");
    CGColorSpaceRelease(_colorSpace);
}

- (void)reset
{
    ReplayLog(@"[SKZRenderDelegate] reset");
    self.needsFrameTexture = NO;
}

- (CIImage*)createImage
{
    ReplayLog(@"[SKZRenderDelegate] createImage called");
    MTLTextureRef metalTexture = nil;
    @synchronized(self.renderedFrameTextures) {
        metalTexture = [self.renderedFrameTextures firstObject];
        if (metalTexture) {
            ReplayLog(@"[SKZRenderDelegate] Retrieved metalTexture");
            [self.renderedFrameTextures removeObjectAtIndex:0];
        } else {
            ReplayLog(@"[SKZRenderDelegate] No metalTexture available");
        }
    }
    
    if (self.busyFrameTexture) {
        @synchronized(self.frameTextures) {
            ReplayLog(@"[SKZRenderDelegate] Moving busyFrameTexture back to frameTextures");
    ...
    if (metalTexture) {
        ReplayLog(@"[SKZRenderDelegate] Creating CIImage from metalTexture");
        ...
        ReplayLog(@"[SKZRenderDelegate] metalTexture was null, returning nil");
    }
    
    ReplayLog(@"[SKZRenderDelegate] returning image %@", image);
...
-(void)startLaunching:(NSNotification*)notification
{
    ReplayLog(@"[SKZRenderDelegate] startLaunching");
    ...
    if (UnitySelectedRenderingAPI() == apiMetal) {
        ReplayLog(@"[SKZRenderDelegate] Is metal, set self to render delegate");
        appDelegate.renderDelegate = self;
        if (renderDelegate) {
            ReplayLog(@"[SKZRenderDelegate] has render delegate, storing");
            self.delegateArray = @[renderDelegate];
        } else {
            ReplayLog(@"[SKZRenderDelegate] has no render delegate");
        }
...
{
    ReplayLog(@"[SKZRenderDelegate] onAfterMainDisplaySurfaceRecreate");
    UnityDisplaySurfaceMTL* surface = (UnityDisplaySurfaceMTL*) GetMainDisplaySurface();
    
    if (!self.frameTextures) {
        ReplayLog(@"[SKZRenderDelegate] Initializing frameTextures and renderedFrameTextures");
        ...
        ReplayLog(@"[SKZRenderDelegate] Surface dimensions: %ldx%ld", (long)width, (long)height);
...
{
    ReplayLog(@"[SKZRenderDelegate] onFrameResolved");
    // In some cases, the ApplicationDidFinishLaunching notif is being sent after
    // the onAfterMainDisplaySurfaceRecreate method is called. This is a workaround
    // to make sure it fully initializes itself, if it hasn't yet.
    if (!self.frameTextures) {
        [self onAfterMainDisplaySurfaceRecreate];
    }
    if (self.needsFrameTexture) {
        if (self.droppedFrames > 10) {
            NSArray *copiedFrames;
            ReplayLog(@"[SKZRenderDelegate] Resetting dropped frames, clearing renderedFrameTextures");
            ...
            @synchronized(self.frameTextures) {
                ...
                if (self.busyFrameTexture) {
                    ReplayLog(@"[SKZRenderDelegate] Busy frame");
                    [self.frameTextures addObject:self.busyFrameTexture];
                    self.busyFrameTexture = nil;
                } else {
                    ReplayLog(@"[SKZRenderDelegate] No busy frame");
                }
            }
            self.needsFrameTexture = NO;
            self.droppedFrames = 0;
        ...
        @synchronized (self.frameTextures) {
            ...
            if (frameTexture) {
                ReplayLog(@"[SKZRenderDelegate] Removing frame from frameTextures");
                [self.frameTextures removeObjectAtIndex:0];
            } else {
                ReplayLog(@"[SKZRenderDelegate] no frame textures");
            }
        }
        
        if (frameTexture) {
            ReplayLog(@"[SKZRenderDelegate] Rendering frame to MTLTexture");
            UnityDisplaySurfaceMTL* surface = (UnityDisplaySurfaceMTL*) GetMainDisplaySurface();
            ...
            ReplayLog(@"[SKZRenderDelegate] Dropping frame");
            self.droppedFrames++;
        }
    }
}
