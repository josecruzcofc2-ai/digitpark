...
namespace SkillzSDK.Internal.Build.iOS
{
	...
	using ActualUnityEditor.iOS.Xcode;
	...
	public static class SkillzPostProcessBuild
	{
		...
		public static void OnPostProcessBuild(ActualUnityEditor.BuildTarget build, string path)
		{
			...
			if (project != null)
			{
                ...
                AddTimestamp(path);
				project.Save();
				IntegrateLiveActivity(build, path);
			}
			else
			{
		...
		private static void IntegrateLiveActivity(ActualUnityEditor.BuildTarget build, string path)
        {
			if (build == ActualUnityEditor.BuildTarget.iOS)
			{
				//Modifying project to add Live Activity Extention
				string projectFilePath = Path.Combine(path, "Unity-iPhone.xcodeproj");
				string frameworkScriptPath = Path.Combine(Application.dataPath, "Skillz", "Internal", "Build", "iOS", "IncludeInXcode", "SkillzLiveActivity", "add_target.rb");
				string builtProductsDir = Path.Combine(Application.dataPath, "Skillz", "Internal", "Build", "iOS", "IncludeInXcode", "SkillzLiveActivity");
				string mainAppTargetName = "Unity-iPhone"; // typically the Unity default
				string bundleId = ActualUnityEditor.PlayerSettings.GetApplicationIdentifier(ActualUnityEditor.BuildTargetGroup.iOS);


				if (!File.Exists(frameworkScriptPath))
				{
					Debug.LogError("Skillz Live Activity script not found.");
					return;
				}
				...
				var process = new System.Diagnostics.Process
				{
					StartInfo = new System.Diagnostics.ProcessStartInfo
					{
						...
						Arguments = $"\"{frameworkScriptPath}\"",
						UseShellExecute = false,
						RedirectStandardOutput = true,
						RedirectStandardError = true
					}
				};
				// Set required environment variables for the Ruby script
				process.StartInfo.EnvironmentVariables["PROJECT_FILE_PATH"] = projectFilePath;
				process.StartInfo.EnvironmentVariables["BUILT_PRODUCTS_DIR"] = builtProductsDir;
				process.StartInfo.EnvironmentVariables["TARGET_NAME"] = mainAppTargetName;
				process.StartInfo.EnvironmentVariables["PROJECT_FOLDER_PATH"] = path;
				process.StartInfo.EnvironmentVariables["BUNDLE_ID"] = bundleId;
				process.StartInfo.EnvironmentVariables["PROVISIONING_PROFILE_SPECIFIER"] = ActualUnityEditor.PlayerSettings.iOS.iOSManualProvisioningProfileID + " Widget";

				process.Start();
				string output = process.StandardOutput.ReadToEnd();
				string error = process.StandardError.ReadToEnd();
				process.WaitForExit();
				Debug.Log("Skillz widget script output:\n" + output);

				//Adding NSSupportsLiveActivities to the main project info.plist
				string plistPath = Path.Combine(path, "Info.plist");
				ActualUnityEditor.iOS.Xcode.PlistDocument plist = new PlistDocument();
				plist.ReadFromFile(plistPath);
				PlistElementDict rootDict = plist.root;
				rootDict.SetBoolean("NSSupportsLiveActivities", true);
				plist.WriteToFile(plistPath);

				//Remove swift files added in IncludeInXcodeProject
				string projPath = ActualUnityEditor.iOS.Xcode.PBXProject.GetPBXProjectPath(path);
				var proj = new ActualUnityEditor.iOS.Xcode.PBXProject();
				proj.ReadFromFile(projPath);

				string unityFrameworkTargetGuid = proj.GetUnityFrameworkTargetGuid();

				// Folder containing Swift files you want to remove
				string swiftFolder = Path.Combine(path, "Libraries", "Skillz", "Internal", "Build", "iOS", "IncludeInXcode", "SkillzLiveActivity");
				UnityEngine.Debug.Log($"🧹 Removed {swiftFolder} from UnityFramework target");
				if (Directory.Exists(swiftFolder))
				{
					foreach (var file in Directory.GetFiles(swiftFolder, "*.swift", SearchOption.AllDirectories))
					{
						string relativePath = file.Replace(path + "/", "");

						string guid = proj.FindFileGuidByProjectPath(relativePath);
						if (!string.IsNullOrEmpty(guid))
						{
							proj.RemoveFileFromBuild(unityFrameworkTargetGuid, guid);
							proj.RemoveFile(guid);
							UnityEngine.Debug.Log($"🧹 Removed {relativePath} from UnityFramework target");
						}
					}
				}
